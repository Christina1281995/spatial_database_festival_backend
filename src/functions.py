...
def connection_params():
    ...
    d1 = input("Step 1 of 4: Your host has been set to 'localhost'. Do you want to change this? "
               "Type 'y' for yes or anything else for no: ")
    if d1 == "Y" or d1 == "y":
        host = input("Please enter a new host and hit enter: ")
    else:
        host = "localhost"
    d2 = input("\nStep 2 of 4: Your database name has been set to 'festival'. Do you want to change this? "
               "Type 'y' for yes or anything else for no: ")
    if d2 == "Y" or d2 == "y":
        database = input("Please enter a new host and hit enter: ")
    else:
        database = "festival"
    d3 = input("\nStep 3 of 4: Your username has been set to 'postgres'. Do you want to change this? "
               "Type 'y' for yes or anything else for no: ")
    if d3 == "Y" or d3 == "y":
...
def connect(host, database, user, password):
    ...
    try:
        ...
        psycopg2.print_psycopg2_exception(err)
...
def check_exists(con, table):
    ...
    cur = con.cursor()
    query = f"SELECT CASE WHEN (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA " \
            f"= 'public' AND  TABLE_NAME = '{str(table)}')) THEN 'TRUE' ELSE 'FALSE' END;"
    try:
        cur.execute(query)
        result = cur.fetchone()
        exists = str(result)
...
def sql_in(con, sql_statement):
    """Tries to execute a statement without any return value from the database.
...
def df_inserts(con, df, table):
    """Performs iterative insert statement into the database with given dataframes
    to set up the tables with existing data."""
    cur = con.cursor()
    rows = len(df)
    for i in range(rows):
...
def setup(con):
    ...
    Checks if PostGIS extension is created. Loads dataframes and inserts them via the df_inserts() function."""
    # ---------------- POSTGIS EXTENSION ----------------
    try:
        cur = con.cursor()
    ...
    tables = {
        "festival_area": "CREATE TABLE IF NOT EXISTS festival_area (id serial NOT NULL PRIMARY KEY, "
                         "geom GEOMETRY(MultiPolygon, 4326));",
        "food_stalls": "CREATE TABLE IF NOT EXISTS food_stalls (id serial NOT NULL PRIMARY KEY, "
                       "name varchar(30) NOT NULL, geom GEOMETRY(MultiPolygon, 4326), cur_staff integer NOT NULL, "
                       "max_staff integer NOT NULL);",
        "food_areas": "CREATE TABLE IF NOT EXISTS food_areas (id serial NOT NULL PRIMARY KEY, "
                      "avg_count integer, cur_count integer, busy_label varchar(15), "
                      "geom GEOMETRY(MultiPolygon, 4326));",
        "user_location": "CREATE TABLE IF NOT EXISTS user_location (id serial NOT NULL, geom GEOMETRY(Point, 4326));",
        "performers": "CREATE TABLE IF NOT EXISTS performers (id serial NOT NULL PRIMARY KEY, "
                      "name varchar(30), genre varchar(20));",
        "stages": "CREATE TABLE IF NOT EXISTS stages (id serial NOT NULL PRIMARY KEY, "
                  "stage_name varchar(20), cur_staff integer, max_staff integer, geom GEOMETRY(Point, 4326));",
        "events": "CREATE TABLE IF NOT EXISTS events (id serial NOT NULL PRIMARY KEY, day integer, "
                  "stage_id integer references stages (id) NOT NULL, "
                  "performer_id integer references performers (id) NOT NULL);",
        "tent_zones": "CREATE TABLE IF NOT EXISTS tent_zones (id serial NOT NULL PRIMARY KEY, "
                      "capacity integer, geom GEOMETRY(MultiPolygon, 4326));",
    ...
    # and stored on GitHub. Please note: all data is made up
    links = [
...
def prepare_plans(con):
    ...
    cur.execute("prepare plan3_3 as "
                "UPDATE food_areas SET busy_label = CASE WHEN cur_count - avg_count > 5 THEN 'high' "
                "WHEN avg_count - cur_count > 5 THEN 'low' ELSE 'average' END WHERE id = $1;")
    cur.execute("prepare plan3_4 as "
                "SELECT busy_label FROM food_areas WHERE id = $1;")
    # Task 4
    cur.execute("prepare plan4 as "
                "SELECT events.id, performers.name, stages.stage_name, stages.max_staff - stages.cur_staff, "
                "stages.max_staff FROM events INNER JOIN performers ON performers.id = events.performer_id INNER JOIN "
                "stages ON stages.id = events.stage_id WHERE day = $1 AND stages.max_staff - stages.cur_staff > 0;")
    # Task 5
    ...
    cur.execute("prepare plan9 as "
                "SELECT events.id, performers.name, stages.stage_name FROM events INNER JOIN performers ON "
                "performers.id = events.performer_id INNER JOIN stages ON stages.id = events.stage_id WHERE day = $1;")
    # Task 10
    cur.execute("prepare plan10 as "
                "SELECT events.day, stages.stage_name, performers.name FROM events INNER JOIN performers ON "
                "performers.id = events.performer_id INNER JOIN stages ON stages.id = events.stage_id "
                "WHERE performers.id = $1;")
    # Task 11
    cur.execute("prepare plan11 as "
                "SELECT e.id, e.name, e.stage_name, (ST_Distance(e.geom::geography, u.geom::geography)::integer) "
                "AS distance FROM (SELECT events.id, performers.name, stages.stage_name, stages.geom FROM events "
                "INNER JOIN performers ON performers.id = events.performer_id INNER JOIN stages ON "
                "stages.id = events.stage_id WHERE day = $1) AS e, user_location AS u ORDER BY distance ASC;")
    # Task 12 doesn't need a plan (it's only select)
    # Task 13 doesn't need a plan (it's only select)
    # Task 14
    cur.execute("prepare plan14 as "
                "SELECT (ST_Distance(t.geom::geography, u.geom::geography)::integer) AS distance FROM "
                "(SELECT geom, id FROM tents WHERE ID = $1) AS t, user_location AS u ORDER BY distance ASC LIMIT 1;")
...
def perform_task(con, task):
    ...
    # Set default show_map to false -> don't load show_map in the end.
    show_a_map = False
    args = {}
    ...
    if task == "1" or task == "1.":
        ...
        not_max_staff_average_busy = "SELECT name, max_staff - cur_staff FROM food_stalls WHERE id IN " \
                                     "(SELECT id FROM food_stalls WHERE cur_staff < max_staff) AND id IN " \
                                     "(SELECT s.id FROM food_stalls AS s, (SELECT geom FROM food_areas WHERE " \
                                     "busy_label = 'average') AS a WHERE ST_Intersects(s.geom, a.geom));"
        cur.execute(not_max_staff_average_busy)
        average = cur.fetchall()
        # Inform User
        print("\nKeep An Eye On stalls that are averagely busy but whose team is not at full capacity:\n")
        for stall in average:
            print(f"- {str(stall[0])}: Send {str(stall[1])} more staff to reach maximum staffing.")
        # busy -> send more staff
        not_max_staff_high_busy = "SELECT name, max_staff - cur_staff FROM food_stalls WHERE id IN " \
                                  "(SELECT id FROM food_stalls WHERE cur_staff < max_staff) AND id IN " \
                                  "(SELECT s.id FROM food_stalls AS s, (SELECT geom FROM food_areas WHERE " \
                                  "busy_label = 'high') AS a WHERE ST_Intersects(s.geom, a.geom));"
        cur.execute(not_max_staff_high_busy)
        busy = cur.fetchall()
        # Inform User
        print("\nMore Staff are definitely needed: \n(These stalls are very busy right now and their "
              "team is not at full capacity)\n")
        for stall in busy:
    ...
    if task == "2" or task == "2.":
        # Update the number of staff members at food stall
        print("Food Stalls: \n")
        # 2.1.
        ...
        # Check input
        if int(stage) in range(1, 20):
            # 2.2.
            cur.execute("execute plan2_2 (%s)" % stage)
            name = cur.fetchone()
            new_count = input(f"\n{str(name[0])} currently has {str(name[1])} staff members. "
                              f"The maximum is {str(name[2])}. What is the new staff count: ")
            # Check if number entered is a number and less than the maximum
            maximum = int(name[2]) + 1
            if new_count.isdigit():
                if int(new_count) < maximum:
                    # 2.3.
                    cur.execute("execute plan2_3 (%s, %s)" % (new_count, stage))
                    con.commit()
                    args[str(name[0])] = f"{str(name[0])} staff: {str(new_count)}"
                    show_a_map = True
                else:
                    print("The value that was entered is above the maximum. Please try again next time!")
            else:
    ...
    if task == "3" or task == "3.":
        ...
        # Check ID
        if int(stage) in range(1, 17):
            new_count = input(f"\nNew visitor count for area {str(stage)}: ")
            # Check new visitor number
            if new_count.isdigit():
                # 3.2.
                cur.execute("execute plan3_2 (%s, %s)" % (new_count, stage))
                con.commit()
                # 3.3.
                cur.execute("execute plan3_3 (%s)" % stage)
                con.commit()
                # 3.4.
                cur.execute("execute plan3_4 (%s)" % stage)
    ...
    if task == "4" or task == "4.":
        ...
        day = input("\nWhich day of the festival is it today? Days range from 1 - 4. "
                    "Enter the day number and hit enter.")
        if int(day) in range(1, 5):
            cur.execute("execute plan4 (%s)" % day)
            result = cur.fetchall()
            print("\nThese events are on today, which need more staff!\n\nEvent ID | Performer | "
                  "Stage | Nr of Staff Needed | Current Staff")
            for row in result:
    ...
    if task == "6" or task == "6.":
        ...
        cur.execute("SELECT name FROM food_stalls WHERE id IN (SELECT s.id FROM food_stalls AS s, "
                    "(SELECT geom FROM food_areas WHERE busy_label = 'low') AS a WHERE ST_Intersects(s.geom, a.geom));")
        result = cur.fetchall()
    ...
    if task == "7" or task == "7.":
        ...
        cur.execute("SELECT name, (ST_Distance(s.geom::geography, u.geom::geography)::integer) "
                    "AS distance FROM food_stalls AS s, user_location AS u ORDER BY distance ASC LIMIT 1;")
        result = cur.fetchone()
    ...
    if task == "8" or task == "8.":
        ...
        cur.execute("SELECT s.name, (ST_Distance(s.geom::geography, u.geom::geography)::integer) "
                    "AS distance FROM food_stalls AS s, user_location as u WHERE s.id IN (SELECT a.id "
                    "FROM food_stalls AS a, food_areas AS b WHERE ST_Intersects(a.geom, b.geom) "
                    "AND b.busy_label = 'low') ORDER BY distance ASC LIMIT 1;")
        result = cur.fetchone()
        print(f"\nThe nearest food stall which isn't busy is {str(result[0])}. It's {str(result[1])} "
              f"meters away from you.\n")
        args["user"] = "Your Position"
        args["Festival Area"] = " "
        args["path"] = str(result[0])
        args[str(result[0])] = f"{str(result[0])} - {str(result[1])} meters away"
    ...
    if task == "10" or task == "10.":
        ...
        # 10.2.
        cur.execute("execute plan10 (%s)" % stage)
        result = cur.fetchone()
        print(f"\nYour favourite artist {str(result[2])} is playing on day {str(result[0])} on "
              f"the {str(result[1])}! Enjoy the show!")
    ...
    if task == "11" or task == "11.":
        ...
        if int(day) in range(1, 5):
            ...
            print(f"\nThese events are happening today. They are ranked by their "
                  f"distance from you (closest to farthest):\n\n")
            i = 1
            for result in results:
                print(f"\t{str(i)}. {str(result[1])} is performing at the {str(result[2])}. "
                      f"It's {str(result[3])} meters away from you.")
    ...
    if task == "12" or task == "12.":
        ...
        cur.execute("SELECT s.stage_name, (ST_Distance(s.geom::geography, u.geom::geography)::integer) "
                    "AS distance FROM stages AS s, user_location AS u ORDER BY distance ASC LIMIT 1;")
        result = cur.fetchone()
    ...
    if task == "13" or task == "13.":
        ...
        cur.execute("SELECT n.id, n.more_tents FROM (SELECT t.id, t.capacity - t.tent_count "
                    "AS more_tents FROM (SELECT tent_zones.id, tent_zones.capacity, COUNT(tents.geom) "
                    "AS tent_count FROM tent_zones LEFT JOIN tents ON ST_Contains(tent_zones.geom,tents.geom) "
                    "GROUP BY tent_zones.id) AS t) AS n WHERE n.more_tents > 0;")
        result = cur.fetchall()
    ...
    if task == "14" or task == "14.":
        ...
        result = cur.fetchone()
        tent_nr = input(f"\nThere are {str(result[0])} tents. The tent numbers therefore "
                        f"range from 1 - {str(result[0])}. Which is your tent number? ")
        # 14.2.
    ...
    return show_a_map, args


def update_position(user_y, user_x, con):
    """Updates the user_location table in the database."""
    ...
    # Else -> update user_location table in postgres
        ...
        sql_in(con, remove)
        # add new position
        add_user_pos = "INSERT INTO user_location(id, geom) VALUES(1, ST_GeomFromText('Point(%s %s)', 4326));" % (
            user_x, user_y)
        sql_in(con, add_user_pos)
...
def show_map(user_y, user_x, show):
    ...
    for key in show:
        for place in locations.places:
            if key == place:
                map_widget.set_marker(locations.places[place][0], locations.places[place][1],
                                      text=f"{str(show[key])}", text_color="black", marker_color_circle="grey50",
                                      marker_color_outside="grey60", font=("Calibri Light", 14))
                ...
                map_widget.set_path([(user_y, user_x), locations.places[show[key]]])
            elif key == "path1":
                map_widget.set_path([(user_y, user_x), locations.places[show[key]]])
            elif key == "path2":
                map_widget.set_path([(user_y, user_x), locations.places[show[key]]])
                ...
                map_widget.set_marker(user_y, user_x, text=f"{str(show[key])}", text_color="darkred",
...
def decide():
    ...
    if task == "7" or task == "7." or task == "8" or task == "8." or task == "11" or task == "11." or task == "12" or \
            task == "12." or task == "14" or task == "14.":
        ...
        print(
            "If you want to simply use one of those, then enter the letter A, B, or C. Otherwise first enter "
            "your Latitude position, hit enter, and then your Longitude position.\n")
        ...
        if user_pos.upper() == "A" or user_pos == "A)":
            ...
            user_y = float(user_pos)
            user_x = float(input("Your position (longitude in format 23.673355): "))
    else:
        user_y = 0
        user_x = 0

    return user_y, user_x, task
